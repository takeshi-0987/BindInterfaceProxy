...
class MacOSNetworkInterface(BaseNetworkInterface):
    """macOS 网络接口实现"""

    def _get_ip_by_iface_name(self, iface_name: str) -> Optional[str]:
        """通过接口名称获取IP地址"""
        try:
            # 使用 ifconfig 命令获取接口IP
        ...
        # 特殊名称处理
        if iface_name == '本地回环' or iface_name == 'lo0':
            return '127.0.0.1'
    ...
    def _get_iface_name_by_ip(self, ip: str) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    # 检测接口行 (如: en0: flags=8863<UP,BROADCAST,SMART,RUNNING,SIMPLEX,MULTICAST> mtu 1500)
                    interface_match = re.match(r'^(\w+): flags=', line)
                    if interface_match:
